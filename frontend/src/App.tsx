...
const defaultCow: Cow = {
  id: "COW_001", name: "Kamadhenu", tagNumber: "IN-TN-7201", breed: "Gir",
...
export default function App() {
  ...
  const [selectedCowId, setSelectedCowId] = useState<string>("COW_001");
  ...
  /* ── Sequential Real-Time Telemetry Loop (No Overlaps, Zero Page Refreshes) ── */
  useEffect(() => {
    let alive = true;
    ...
    const pollStream = async () => {
      ...
      try {
        ...
        if (live && live.length > 0) {
          ...
          // Trigger alerts automatically for high-risk cattle
          live.forEach((cow) => {
              ...
              setAlerts((prev) => [
                {
                  ...
                  message: `${cow.name || cow.id} — temp ${cow.temperature}°C, abnormal swelling/conductivity detected.`,
        ...
        if (alive) {
          // Poll sequentially every 1200ms after previous request resolves
          timerId = setTimeout(pollStream, 1200);
        }
      }
    };
  ...
  useEffect(() => {
    fetchSmsHistory().then(setSmsHistory).catch(() => {});
  }, []);

  // Always resolve selected cow to live array first, then fallback
  ...
  const handleSelectCow = (cow: Cow) => {
    ...
    setCurrentScreen("cow_detail");
  };

  const handleAnalyzeWithAi = async (cowId: string) => {
    setSelectedCowId(cowId);
    setCurrentScreen("ai_intelligence");
    setIsAnalyzing(true);
    try {
      const u = await analyzeCowWithAi(cowId);
      setCows((p) => p.map((c) => (c.id === u.id ? u : c)));
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleToggleQuarantine = async (cowId: string, q: boolean) => {
    try {
      await toggleCowQuarantine(cowId, q);
      setCows((p) => p.map((c) => (c.id === cowId ? { ...c, isQuarantined: q } : c)));
    } catch (e) {
      console.error(e);
    }
  };
      ...
      case "dashboard":
        return (
          <DashboardScreen
            ...
            onNavigateToHerd={() => navigate("herd")}
            onNavigateToAlerts={() => navigate("alerts")}
            onNavigateToAiStudio={handleAnalyzeWithAi}
            onNavigateToSms={() => navigate("sms_warning")}
          />
        );
      case "herd":
        return (
          <HerdScreen
            ...
            onOpenAddCow={() => setIsAddCowOpen(true)}
          />
        );
      case "cow_detail":
        return (
          <CowDetailScreen
            cow={selectedCow}
            currentLang={currentLang}
            onBack={() => navigate("herd")}
            onToggleQuarantine={handleToggleQuarantine}
            onOpenSmsForCow={(c) => {
              setSelectedCowId(c.id);
              navigate("sms_warning");
            }}
            onOpenExamForCow={() => setIsExamModalOpen(true)}
            onAnalyzeWithAi={handleAnalyzeWithAi}
          />
        );
      case "ai_intelligence":
        return (
          <AiIntelligenceScreen
            ...
            currentLang={currentLang}
            onSelectCow={handleSelectCow}
            onAnalyzeCow={handleAnalyzeWithAi}
      ...
      case "sms_warning":
        return (
          <SmsWarningScreen
            ...
            onUpdateSmsSettings={setSmsSettings}
            onDispatchSms={handleSendLiveSms}
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex font-sans">
              ...
              {(["en", "ta", "hi"] as LanguageCode[]).map((lang) => (
                <button
                  ...
                  className={`px-2.5 py-0.5 rounded-full transition-all ${
                    currentLang === lang ? "bg-[#00361A] text-white shadow-xs" : "text-slate-500"
                  }`}
                >
      ...
      <AddCowModal
        isOpen={isAddCowOpen}
        onClose={() => setIsAddCowOpen(false)}
        currentLang={currentLang}
        onAddCow={(data) => {
          const n = cows.length + 1;
          const id = `COW_${n.toString().padStart(3, "0")}`;
          setCows((p) => [
            ...p,
            {
              ...defaultCow,
              id,
              name: data.name || `Cow #${n}`,
              tagNumber: `IN-TN-${7200 + n}`,
              breed: data.breed || "Jersey Cross",
      ...
      <FarmerHelpModal
        ...
        currentLang={currentLang}
        emergencyHotline={smsSettings.emergencyVetHotline}
      />
      ...
      <SmsEarlyWarningToast
        cow={toastCow}
        onDismiss={() => setToastCow(null)}
        onIsolateAndDispatch={handleIsolateAndDispatch}
      />
    </div>
  );
